...
contract CoachAITreasury is CoachAIAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    /**
     * @notice allow approved address to deposit an asset for CADT
    ...
    function deposit(
        ...
        // mint CADT needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        CADT.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn CADT for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        CADT.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new CADT using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        CADT.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or CADT from the treasury, using sCADT as collateral. This might allow an
     * sCADT holder to provide CADT liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(CADT)) {
            require(permissions[STATUS.CADTDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sCADT.changeDebt(value, msg.sender, true);
        require(sCADT.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(CADT)) {
            CADT.mint(msg.sender, value);
            cadtDebt = cadtDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sCADT.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with CADT
     * @param _amount uint256
     */
    function repayDebtWithCADT(uint256 _amount) external {
        require(
            permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.CADTDEBTOR][msg.sender],
            notApproved
        );
        CADT.burnFrom(msg.sender, _amount);
        sCADT.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        cadtDebt = cadtDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(CADT), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SCADT) {
            sCADT = IsCADT(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SCADT) {
            // 9
            sCADT = IsCADT(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(CADT.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns CADT valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(CADT)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return CADT.totalSupply() - cadtDebt;
    }
}
