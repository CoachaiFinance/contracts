...
contract CoachAIBondingCalculator is IBondingCalculator {
    using FixedPoint for *;
    using SafeMath for uint256;

    IERC20 internal immutable CADT;
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == address(CADT)) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair(_pair).token1() == address(CADT), "Invalid pair");
            reserve = reserve0;
        }
        return reserve.mul(2 * (10**IERC20Metadata(address(CADT)).decimals())).div(getTotalValue(_pair));
    }
}
