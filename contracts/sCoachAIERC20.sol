...
contract sCoachAI is IsCADT, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgCADT public gCADT; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked CADT", "sCADT", 18) ERC20Permit("Staked CADT") {
    ...
    function setgCADT(address _gCADT) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gCADT) == address(0), "gCADT:  gCADT already set");
        require(_gCADT != address(0), "gCADT:  gCADT is not a valid contract");
        gCADT = IgCADT(_gCADT);
    }
    ...
    /**
        @notice increases rCADT supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sCADT of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sCADT
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sCADT: insufficient balance");
    }
    ...
    // toG converts an sCADT balance to gCADT terms. gCADT is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gCADT.balanceTo(amount);
    }

    // fromG converts a gCADT balance to sCADT terms. sCADT is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gCADT.balanceFrom(amount);
    }

    // Staking contract holds excess sCADT
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gCADT.balanceFrom(IERC20(address(gCADT)).totalSupply())).add(
