...
/// @author Zapper and CoachAIDAO
/// @notice This contract enters CoachAI Pro bonds
...
contract CoachAIPro_Zap_V1 is ZapBaseV2_2 {
    ...
    // IE DAI => wanted payout token (IE CADT) => bond depo
    mapping(address => mapping(address => address)) public principalToDepository;

    // If a token can be paid out by CoachAI Pro
    mapping(address => bool) public isCoachAIProToken;
    ...
    constructor(
        ...
        coachaiDAO = _coachaiDAO;

        transferOwnership(ZapperAdmin);
    }

    /**
     * @notice This function deposits assets into CoachAIDAO with ETH or ERC20 tokens
     * @param fromToken The token used for entry (address(0) if ether)
     * @param amountIn The amount of fromToken to invest
     * @param toToken The token fromToken is getting converted to.
     * @param minToToken The minimum acceptable quantity sCADT or wsCADT or principal tokens to receive. Reverts otherwise
     ...
     * @return bondTokensRec quantity of sCADT or wsCADT  received (depending on toToken) or the quantity CADT vesting (if bond is true)
     */
    function ZapIn(
        address fromToken,
        uint256 amountIn,
        ...
        require(isCoachAIProToken[bondPayoutToken], "fromToken must be bondable using OP");
    ...
    function update_CoachAIDAO(address _coachaiDAO) external onlyCoachAIDAO {
        coachaiDAO = _coachaiDAO;
    }

    string private ARRAY_LENGTH_ERROR = "array param lengths must match"; // save gas

    function update_isCoachAIProToken(address[] memory _tokens, bool[] memory _isToken)
        external
        onlyCoachAIDAO
    {
        require(_tokens.length == _isToken.length, ARRAY_LENGTH_ERROR);
        for (uint256 i; i < _tokens.length; i++) {
            isCoachAIProToken[_tokens[i]] = _isToken[i];
        }
    }
    ...
    function update_BondDepos(
        ...
        for (uint256 i; i < principals.length; i++) {
            require(isCoachAIProToken[payoutTokens[i]], "payoutTokens must be on OP");
