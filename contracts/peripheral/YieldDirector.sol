...
import {IsCADT} from "../interfaces/IsCADT.sol";
import {SafeERC20} from "../libraries/SafeERC20.sol";
import {IYieldDirector} from "../interfaces/IYieldDirector.sol";
import {CoachAIAccessControlled, ICoachAIAuthority} from "../types/CoachAIAccessControlled.sol";

/**
    @title YieldDirector (codename Tyche) 
    @notice This contract allows donors to deposit their sCADT and donate their rebases
            to any address. Donors will be able to withdraw their principal
            sCADT at any time. Donation recipients can also redeem accrued rebases at any time.
 */
contract YieldDirector is IYieldDirector, CoachAIAccessControlled {
    ...
    struct DonationInfo {
        ...
        uint256 carry; // Amount of sCADT accumulated over on deposit/withdraw
    ...
    constructor(address sCadt_, address authority_) CoachAIAccessControlled(ICoachAIAuthority(authority_)) {
        require(sCadt_ != address(0), "Invalid address for sCADT");
    ...
    /**
        @notice Deposit sCADT, records sender address and assign rebases to recipient
        @param amount_ Amount of sCADT debt issued from donor to recipient
    ...
    */
    function deposit(uint256 amount_, address recipient_) external override {
        ...
        IERC20(sCADT).safeTransferFrom(msg.sender, address(this), amount_);

        uint256 index = IsCADT(sCADT).index();
    ...
    /**
        @notice Withdraw donor's sCADT from vault and subtracts debt from recipient
     */
    function withdraw(uint256 amount_, address recipient_) external override {
        ...
        IERC20(sCADT).safeTransfer(msg.sender, amount_);
    ...
    function withdrawAll() external override {
        ...
        uint256 sCadtIndex = IsCADT(sCADT).index();
        uint256 total = 0;
        ...
        for (uint256 index = 0; index < donationsLength; index++) {
            ...
            recipient.agnosticDebt = _toAgnostic(recipient.totalDebt + recipient.carry);
            recipient.indexAtLastChange = sCadtIndex;
        ...
        IERC20(sCADT).safeTransfer(msg.sender, total);

        emit AllWithdrawn(msg.sender, total);
    }

    /**
        @notice Get deposited sCADT amount for specific recipient
     */
    ...
    /**
        @notice Return total amount of donor's sCADT deposited
     */
    ...
    /**
        @notice Return total amount of sCADT donated to recipient since last full withdrawal
     */
    ...
    /**
        @notice Return total amount of sCADT donated from donor since last full withdrawal
     */
    ...
    /**
        @notice Get redeemable sCADT balance of a recipient address
     */
    ...
    /**
        @notice Redeem recipient's full donated amount of sCADT at current index
        @dev Note that a recipient redeeming their vault shares effectively pays back all
             sCADT debt to donors at the time of redeem. Any future incurred debt will
    ...
    function redeem() external override {
        ...
        recipient.carry = 0;
        recipient.indexAtLastChange = IsCADT(sCADT).index();

        IERC20(sCADT).safeTransfer(msg.sender, redeemable);
    ...
    /**
        @notice Get accumulated sCADT since last time agnostic value changed.
     */
