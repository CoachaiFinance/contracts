...
interface IClaim {
    struct Term {
        ...
        uint256 max; // maximum nominal CADT amount can claim
    }
...
/**
 *  This contract allows CoachAI genesis contributors to claim CADT. It has been
...
contract GenesisClaim is Ownable {
    ...
    // claim token
    IERC20 internal immutable cadt = IERC20(0x64aa3364F17a4D01c6f1751Fd97C2BD3D7e7f1D5);
    // payment token
    ...
    IgCADT internal immutable gCADT = IgCADT(0x0ab87046fBb341D058F17CBC4c1133F25a20a52f);
    ...
    /**
     * @notice allows wallet to claim CADT
     * @param _to address
     * @param _amount uint256
     */
    function claim(address _to, uint256 _amount) external {
        cadt.safeTransfer(_to, _claim(_amount));
    }

    /**
     * @notice allows wallet to claim CADT and stake. set _claim = true if warmup is 0.
     * @param _to address
     * @param _amount uint256
    ...
    /**
     * @notice logic for claiming CADT
     * @param _amount uint256
    ...
    function _claim(uint256 _amount) internal returns (uint256 toSend_) {
        ...
        if (useStatic) {
            terms[msg.sender].gClaimed = info.gClaimed.add(gCADT.balanceTo(toSend_.mul(9).div(10)));
            terms[msg.sender].claimed = info.claimed.add(toSend_.div(10));
        } else terms[msg.sender].gClaimed = info.gClaimed.add(gCADT.balanceTo(toSend_));
    }
    ...
    function approve() external {
        cadt.approve(address(staking), 1e33);
    ...
    /**
     * @notice view CADT claimable for address. DAI decimals (18).
    ...
    /**
     * @notice view CADT claimed by address. CADT decimals (9).
     * @param _address address
     * @return uint256
     */
    function claimed(address _address) public view returns (uint256) {
        return gCADT.balanceFrom(terms[_address].gClaimed).add(terms[_address].claimed);
    }

    /**
     * @notice view circulating supply of CADT
     * @notice calculated as total supply minus DAO holdings
     * @return uint256
     */
    function circulatingSupply() public view returns (uint256) {
        return treasury.baseSupply().sub(cadt.balanceOf(dao));
    }
