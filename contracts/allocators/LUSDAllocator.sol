...
contract LUSDAllocator is CoachAIAccessControlled {
    /* ======== DEPENDENCIES ======== */
    ...
    ITreasury public treasury; // CoachAI Treasury
    ...
    function withdraw(address token, uint256 amount) external onlyGuardian {
        ...
        if (token == lusdTokenAddress) {
            ...
            treasury.deposit(balance, token, value); // deposit using value as profit so no CADT is minted
        } else {
    ...
    /**
    Helper method copying CoachAITreasury::_tokenValue(), whose name was 'valueOf()' in v1 
