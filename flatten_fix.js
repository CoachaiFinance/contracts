...
fs.readdir(root, function (err, files) {
  ...
  files.forEach(function (file, index) {
    fs.readFile(root + file, 'utf8', function (err,data) {
      ...
      result = result.replace(/\/\/ SPDX-License-Identifier: AGPL-3.0-or-later/g, '');
      result = result.replace(/\/\/ SPDX-License-Identifier: AGPL-3.0-only/g, '');
      result = result.replace(/\/\/ SPDX-License-Identifier: AGPL-3.0/g, '');
      result = result.replace(/\/\/ SPDX-License-Identifier: WTFPL/g, '');
      result = result.replace(/\/\/ SPDX-License-Identifier: GPL-2.0/g, '');
